...
export const voiceApi = {
  ...
  getAll: async (provider?: string): Promise<Voice[]> => {
    try {
      ...
      const response = await apiClient.get<VoiceResponse>('/admin/voices/list', { params });
  ...
  getById: async (voiceId: string): Promise<Voice> => {
    try {
      const response = await apiClient.get<VoiceDetailResponse>(`/admin/voices/get/${voiceId}`);
